...
def convert_deci_reo(nombre) :
    ...
    for i in range(len(nombre)):
        ...
        if 
        if puissance > 1 and nombre[i] > "0" and nombre[i+1] != "0":
            reo += " " + numera[nombre[i]] + " " + puissance_dix[str(puissance)] + " e "
            
        elif puissance > 1 and nombre[i] > "0" and nombre[i-1] == "0":
            reo += numera[nombre[i]] + " " + puissance_dix[str(puissance)]
            
        elif puissance > 0 and nombre[i] != "0":
            pass
        
        elif puissance == 1 and i != "0" and nombre[i+1] != "0":
            reo += numera[nombre[i]] + " " + puissance_dix[str(puissance)] + " ma "
        """    
        elif puissance == 1 and i == "1" and nombre[i+1] != "0":
